"""
与dataset1的差别在于, 这里没有shfulle这个操作算子
"""
import pickle
import jieba
from process import *
...
def shuffle(dataset):
    ...
    :return: 
    """
    buckets_instances_indexs = dataset.buckets_instances_indexs
    for id, length in dataset.buckets_instances_numbers.items():
        dataset.buckets_points[id] = 0
        np.random.shuffle(buckets_instances_indexs[id])
        print("shuffle success!!")
    return buckets_instances_indexs
...
if __name__ == "__main__":
    ...
    buckets_instances_indexs = shuffle(dataset)  # 每一次shuffle就产生一个共用的buckets_instances_indexs 给其他的数据集使用
    print(buckets_instances_indexs)
