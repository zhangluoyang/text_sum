...
def parse_args():
    ...
    parser.add_argument('--epoch', help='numbers of epoch, default=100 ', default=100, type=int)
    ...
    parser.add_argument('--content', help='content data file', default="data/contents.txt", type=str)
    parser.add_argument('--title', help='title data file', default="data/titles.txt", type=str)
    ...
    parser.add_argument('--train_or_test', help='train, test or train_test, default=train', default="train_test", type=str)
    return parser.parse_args()  # 这里需要特别留意 python2和python3是有一些区别的

def train(buckets, batch_size, content, title, size, num_layers, do_decoder, num_gpus, exists, epoch, lr, min_lr, max_grad_norm):
    """
    :param buckets: 桶
    :param batch_size:　 
    ...
    model.fit(dataset=dataset, lr=lr, min_lr=min_lr, max_grad_norm=max_grad_norm, epoch = epoch)

def test():
    params = json.load(open("conf/params.json", "r"))
    ...
    checkpoint_file = tf.train.latest_checkpoint(checkpoint_dir)  # 这里取出的是最后一个model文件
...
def main(args):
    ...
    if trian_or_test == 'train':
        train(buckets, batch_size, content, title, size, num_layers, do_decoder, num_gpus, exists, epoch, lr, min_lr, max_grad_norm)
