...
class DataSet(object):
    ...
    def __init__(self, train_data, test_data, batch_size, data_exists, buckets, content_word_lengths, content_max_length=200, title_max_length=40):
        """
        :param train_data: 训练集
        :param test_data:  测试集
        :param batch_size:
        :param data_exists: 数据是否已经存在
        :param buckets: 桶 解码输出部分 这里不会出现大于title_max_length的情况 必须要比title_max_length多１
        :param content_word_lengths: content的长度
    ...
    def process(self, content_max_length=200, title_max_length=40):
        ...
        with open("data/titles.txt", "r", encoding="utf-8") as f:
            titles = f.readlines()
        with open("data/contents.txt", "r", encoding="utf-8") as f:
        ...
        pickle.dump(all_words, open("process_data/all_words", "wb"))
        pickle.dump(words_id, open("process_data/words_id", "wb"))
        pickle.dump(instances_titles, open("process_data/instances_titles", "wb"))
        pickle.dump(instances_contents, open("process_data/instances_contents", "wb"))
    ...
    def load(self):
        all_words = pickle.load(open("process_data/all_words", "rb"))
        words_id = pickle.load(open("process_data/words_id", "rb"))
        instances_titles = pickle.load(open("process_data/instances_titles", "rb"))
        instances_contents = pickle.load(open("process_data/instances_contents", "rb"))
        vocab_size = len(all_words)
        self.all_words = all_words
        self.vocab_size = vocab_size
        self.words_id = words_id
...
class DataConvert(object):
    ...
    数据的转换 用于模型的预测阶段
    """
    def __init__(self, buckets, content_word_lengths, content_max_length=200, title_max_length=40):
        """

        :param buckets: 桶
        :param content_word_lengths: 内容长度
        :param content_max_length: 最大长度
        :param title_max_length: 标题最大长度
        """
        self.buckets = buckets
        self.content_word_lengths = content_word_lengths
...
if __name__ == "__main__":
    ...
    id, encoder_length, encoder_inputs_ids, decoder_inputs_ids = convert.convert("我爱你中国")
    print(id)
    print(encoder_length)
    print(encoder_inputs_ids)
    print(decoder_inputs_ids)
    # dataset = DataSet("data/contents.txt", "data/titles.txt", batch_size, True, buckets, content_word_lengths,
